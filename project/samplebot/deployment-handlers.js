  ...
  return co(function* () {
    if (!baseTemplate) {
      const objects = yield s3.listObjects({
        Bucket: ServerlessDeploymentBucket.name,
        // Bucket: 'tradle-dev-serverlessdeploymentbucket-nnvi6x6tiv7k',
        Prefix: artifactDirectoryPrefix
      }).promise()

      const metadata = objects.Contents.find(object => object.Key.endsWith(templateFileName))
      if (!metadata) {
...
function generateTemplate ({ resources, template, parameters }) {
  ...
  // set org env
  ;[
    getLambdaEnv(Resources.InfoLambdaFunction),
    getLambdaEnv(Resources.PreauthLambdaFunction)
  ].forEach(env => {
    env.ORG_NAME = name
...
co(function* () {
  ...
  console.log(`https://${PublicConfBucket.name}.s3.amazonaws.com/${templateKey}`)
})().catch(console.error)

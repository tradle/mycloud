...
function createMessagesTable({ models, tables }) {
    const model = models['tradle.Message'];
    const inbox = dynamodb_1.createTable(definitions.InboxTable.TableName, {
        models,
        model,
        exclusive: true,
        forbidScan: true,
        tableDefinition: dynamodb_1.utils.toDynogelTableDefinition(definitions.InboxTable)
    });
    const outbox = dynamodb_1.createTable(definitions.OutboxTable.TableName, {
        models,
        model,
        exclusive: true,
        forbidScan: true,
        tableDefinition: dynamodb_1.utils.toDynogelTableDefinition(definitions.OutboxTable)
    });

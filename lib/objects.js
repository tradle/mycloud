...
class Objects {
    constructor(tradle) {
        ...
        this.validateNewVersion = (opts) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (previous[constants_1.OWNER]) {
                const { _author } = yield getNewAuthorInfo;
                if (![].concat(previous[constants_1.OWNER]).includes(_author)) {
                    throw new errors_1.InvalidAuthor(`expected ${previous[constants_1.OWNER]} as specified in the previous verison's ${constants_1.OWNER} property, got ${_author}`);
                }
            }
            const getOldAuthor = previous._author ? Promise.resolve(previous) : identities.getAuthorInfo(previous);
            const [newInfo, oldInfo] = yield Promise.all([getNewAuthorInfo, getOldAuthor]);
            if (newInfo._author !== oldInfo._author) {
                throw new errors_1.InvalidAuthor(`expected ${oldInfo._author}, got ${newInfo._author}`);
            }
            try {

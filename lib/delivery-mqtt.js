    ...
    constructor({ env, iot, auth, messages, objects, logger }) {
        ...
        this.deliverBatch = ({ session, recipient, messages }) => __awaiter(this, void 0, void 0, function* () {
            if (!(session.authenticated && session.connected)) {
                const status = JSON.stringify(utils_1.pick(session, ['authenticated', 'connected']));
            ...
            const subBatches = utils_2.batchStringsBySize(strings, MAX_PAYLOAD_SIZE);
            const promises = [];
...
const stringify = msg => JSON.stringify(utils_2.omitVirtual(msg));

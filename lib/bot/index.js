...
const { extend, pick } = require('../utils');
const { addLinks } = require('../crypto');
const { getMessagePayload, getMessageGist, ensureTimestamped, normalizeSendOpts, normalizeRecipient } = require('./utils');
...
exports.createBot = (opts = {}) => {
    return exports.fromEngine(Object.assign({}, opts, { tradle: opts.tradle || require('../').tradle }));
};
function createBot(opts = {}) {
    let { autosave = true, resources, models, send, sign, seals, env = {}, lambdaUtils } = opts;
    ...
    bot.send = (opts) => __awaiter(this, void 0, void 0, function* () {
        ...
        let messages;
        try {
            messages = yield send(batch);
        }
        finally {
            outboundMessageLocker.unlock(recipient);
        }
        if (TESTING && messages) {
            yield Promise.all(messages.map(message => savePayloadToTypeTable(clone(message))));
        }
        if (messages) {

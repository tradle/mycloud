...
const COPY_TO_BOT = [
  'aws', 'models', 'objects', 'db', 'seals', 'seal',
...
function createBot (opts={}) {
  ...
  const processEvent = co(function* (eventName, event) {
    let { payload } = event
    yield promiseReady
    try {
      // waterfall to preprocess
      payload = yield preProcessHooks.waterfall(eventName, payload)
      // bubble to allow handlers to terminate processing
      const result = yield hooks.bubble(eventName, payload)
      yield postProcessHooks.fire(eventName, payload, result)
    } catch (error) {
      logger.error(`failed to process ${eventName}`, {
        event,
        payload,
        error: error.stack
      })

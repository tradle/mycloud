...
function createBot (opts={}) {
  ...
  const debug = env.logger('bot-engine')
  ...
  bot.send = co(function* (opts) {
    ...
    if (TESTING) {
      yield savePayload(message)
    }

    return message
  })

  const savePayload = co(function* (payload) {
    if (!payload) {
      debug('no payload to save...')
      return
    }

    while (payload[TYPE] === 'tradle.Message') {
      payload = payload.object
    }

    try {
      yield bot.save(payload)
      debug('saved payload directly', payload[TYPE])
    } catch (err) {
      debug('failed to save payload directly', payload[TYPE], err.stack)
    }
  })
  ...
  const messageProcessingLocker = locker({
    ...
    debug: env.logger('message-locker'),
  ...
  const normalizeOnMessageInput = co(function* (message) {
    ...
    debug(`receiving ${JSON.stringify(getMessageGist(message), null, 2)}`)
    return {
      bot,
      user,
      message,

...
function setupGraphQL(bot) {
    let { logger, router, objects, models, db, promiseReady } = bot;
    ...
    if (logger.level >= logger_1.Level.SILLY) {
        gqlRouter.use(utils_1.logResponseBody(logger));
    }
    gqlRouter.use('/', expressGraphQL((req) => __awaiter(this, void 0, void 0, function* () {
        yield promiseReady();

...
module.exports = function createBotInputs({ models, tradle }) {
    const { env, provider, seals, identities, objects, messages, aws, db, kv, conf, contentAddressedStorage, resources, tables, buckets, router, init, wrap, friends, lambdaUtils, version, apiBaseUrl } = tradle;
    const { docClient } = aws;
    if (models) {
        db.addModels(models);
    }

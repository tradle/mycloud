...
const defaultBackoffFunction = (retryCount) => {
...
function createDBUtils({ aws, logger }) {
    ...
    function getTable(TableName) {
        ...
        tableAPI.batchProcess = (_a) => {
            var { params = {} } = _a, opts = __rest(_a, ["params"]);
            return batchProcess(Object.assign({ params: Object.assign({}, params, { TableName }) }, opts));
        };
        tableAPI.clear = () => clear(TableName);
        tableAPI.getTableDefinition = () => getTableDefinition(TableName);
        return tableAPI;
    }
    const execWhile = (method, params, filter) => __awaiter(this, void 0, void 0, function* () {
        while (true) {
            try {
                return yield exec(method, params);
            }
            catch (err) {
                if (!filter(err))
                    throw err;
    ...
    const batchProcess = ({ params, processOne, processBatch }) => __awaiter(this, void 0, void 0, function* () {
        ...
        while (keepGoing && (lastEvaluatedKey || retry)) {
            try {
                response = yield aws.docClient[method](params).promise();
            }
            catch (err) {
                if (err.retryable) {
                    retry = true;
                    yield utils_1.waitImmediate();
                    continue;
                }
                retry = false;
                throw err;
            ...
            if (processBatch) {
                keepGoing = yield processBatch(response);
            }
            else {
                const results = yield Promise.all(response.Items.map(item => processOne(item)));
                keepGoing = results.every(result => result !== false);
            }
        }
    });
    const getTableDefinition = (TableName) => __awaiter(this, void 0, void 0, function* () {
        if (definitions[TableName])
            return definitions[TableName];
        const { Table } = yield aws.dynamodb.describeTable({ TableName }).promise();
        return Table;
    });
    const clear = (TableName, filter = alwaysTrue) => __awaiter(this, void 0, void 0, function* () {
        const { KeySchema } = yield getTableDefinition(TableName);
    ...
    const get = (params) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (!result.Item) {
            throw new errors_1.NotFound(JSON.stringify(_.pick(params, ['TableName', 'Key'])));
        }
        return result.Item;
    });
...
exports.createDBUtils = createDBUtils;

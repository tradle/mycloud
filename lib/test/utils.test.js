...
test('ModelStore', utils_1.loudAsync((t) => __awaiter(this, void 0, void 0, function* () {
    ...
    t.equal(yield store.getCumulativeForeignModelsPack(), null);
    console.log('patience...');
    yield store.addModelsPack({ modelsPack });
    t.same(yield store.getCumulativeForeignModelsPack(), modelsPack);
    ...
    const allForeign = yield store.getCumulativeForeignModelsPack();
    ...
    const cumulative = yield store.getCumulativeModelsPack();
    ...
    t.equal(isCumulative, true);
    const schema = yield store.getSavedGraphqlSchema();
    t.ok(schema);
    t.end();
})));
function values(obj) {

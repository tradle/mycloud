...
function E2ETest({ tradle = defaultTradleInstance, models, products, bot, productsAPI, employeeManager }) {
    this.bot = bot;
    this.tradle = tradle;
...
proto.onboardCustomer = co(function* ({ user, relationshipManager, product }) {
    const { bot } = this;
...
proto.judge = co(function* ({ employee, user, application, context, approve = true }) {
    const { bot, productsAPI } = this;
    if (application) {
...
proto.runThroughApplication = co(function* ({ user, awaitCertificate, product, relationshipManager }) {
    const { productsAPI, employeeManager } = this;
...
const clearBuckets = ({ tradle }) => __awaiter(this, void 0, void 0, function* () {
    yield Promise.all(Object.keys(tradle.buckets).map((id) => __awaiter(this, void 0, void 0, function* () {

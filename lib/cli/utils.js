...
const interpolateTemplate = (opts={}) => {
  ...
  if (sync) return proc.execSync(command).toString()
...
const compileTemplate = co(function* (path) {
  ...
  const interpolated = YAML.load(interpolatedStr)
...
const getNativeModules = co(function* (dir='node_modules', modules={}) {
...
const getProductionModules = co(function* () {
  const command = 'npm ls --production --parseable=true --long=false --silent'
  const buf = yield proc.exec(command, {
    cwd: process.cwd()
  })
...
const downloadDeploymentTemplate = co(function* () {

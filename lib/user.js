...
proto.onSubscribed = co(function* ({ clientId, topics }) {
  this.debug('client subscribed to topics:', topics.join(', '))
...
proto.onSentMessage = co(function* ({ clientId, message }) {
  ...
  try {
    ...
    this.debug('failed to process inbound message:', JSON.stringify(message, null, 2), e.stack)
    // delivery http
    if (!clientId) throw e
  ...
  if (processed) {
    // SUCCESS!
    this.debug('received valid message from user')
    ...
    if (!BOT_ONMESSAGE) {
      this.debug('no bot subscribed to "onmessage"')
      return
    }

    // const { author, time, link } = wrapper.message
    const neutered = this.messages.stripData(processed)
    this.debug(`passing message from ${processed._author} on to bot`)
  ...
  this.debug('processing error in receive:', err.name)
  processed = err.progress
  if (err instanceof Errors.Duplicate) {
    this.debug('ignoring but acking duplicate message', prettify(processed))
    // HTTP
    if (!clientId) return

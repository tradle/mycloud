...
function createProductsBot({ bot, conf }) {
    ...
    const sendVerifications = ({ req, user, application }) => __awaiter(this, void 0, void 0, function* () {
        if (req) {
            if (!user)
                user = req.user;
            if (!application)
        ...
        const { forms, verificationsImported = [], verificationsIssued = [] } = application;
        const unverified = forms.filter(form => {
            return !verificationsIssued.find(({ item }) => {
                return item.id === form.id;
            });
        });
        yield Promise.all(unverified.map(formStub => {

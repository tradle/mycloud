...
function createProductsBot({ bot, namespace, conf, customModels, style, event }) {
    ...
    if (handleMessages) {
        ...
        const employeeModels = lodash_1.omit(productsAPI.models.all, BASE_MODELS_IDS);
        const customerModels = lodash_1.omit(productsAPI.models.all, Object.keys(productsAPI.models.private.all)
        ...
        if (tours) {
            const { intro } = tours;
            if (intro) {
                const sendLatestTour = keep_fresh_1.keepFreshPlugin({
                    object: intro,

...
class Seals {
    constructor({ provider, blockchain, network, tables, db, objects, env }) {
        ...
        this._requeueWrites = (seals) => __awaiter(this, void 0, void 0, function* () {
            if (!seals.length)
                return;
            this.logger.debug('failed writes', seals.map(seal => utils_1.pick(seal, ['timeSealed', 'txId'])));
            const now = utils_1.timestamp();
            const puts = seals.map(seal => {
                return Object.assign({}, utils_1.omit(seal, ['unconfirmed', 'txId']), { unsealed: String(now), txId: null });
            });
        ...
        this._cancelReads = (seals) => __awaiter(this, void 0, void 0, function* () {
            if (!seals.length)
                return;
            this.logger.debug('failed reads', seals.map(seal => utils_1.pick(seal, ['address', 'link'])));
            const now = utils_1.timestamp();
            const puts = seals.map(seal => {
                return Object.assign({}, utils_1.omit(seal, 'unconfirmed'), { unwatched: String(now) });
            });
            yield this.table.batchPut(puts);
        });
        this._syncUnconfirmed = (opts = {}) => __awaiter(this, void 0, void 0, function* () {
            ...
            const updateObjectsAndDB = Promise.all(links.map((link) => __awaiter(this, void 0, void 0, function* () {
                ...
                yield Promise.all([
                    this.db.update(Object.assign({}, utils_1.pick(object, [constants_1.TYPE, '_time', '_link', '_permalink', '_virtual']), { _seal: sealResource })),

...
const yn = require('yn')
const TESTING = process.env.NODE_ENV === 'test' || yn(process.env.IS_OFFLINE)
if (TESTING) {
  require('../test/env')
} else {
...
const lambdaShortName = AWS_LAMBDA_FUNCTION_NAME
...
const debug = createDebugger(`λ:${lambdaShortName}`)
...
env.debugger = namespace => createDebugger(`λ:${lambdaShortName}:${namespace}`)

env.setFromLambdaEvent = (event, context) => {
  env.IS_WARM_UP = event.source === 'serverless-plugin-warmup'
...
env.NO_TIME_TRAVEL = yn(env.NO_TIME_TRAVEL)

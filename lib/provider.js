...
class Provider {
    constructor(tradle) {
        ...
        this._sendMessageBatch = (batch) => __awaiter(this, void 0, void 0, function* () {
            ...
            const promiseCreates = utils_1.series(batch.map(sendOpts => () => this.createSendMessageEvent(sendOpts)));
            ...
            const session = yield promiseSession;
            const friend = yield promiseFriend;
            const messages = yield promiseCreates;
            yield this.attemptLiveDelivery({ recipient, messages, session, friend });
            return messages;
        });

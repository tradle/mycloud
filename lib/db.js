...
module.exports = function createDB(opts) {
    ...
    db.setExclusive({
        model: pubKeyModel,
        table: dynamodb_1.createTable(pubKeysDef.TableName, Object.assign({}, commonOpts, { exclusive: true, readOnly: true, model: pubKeyModel, tableDefinition: dynamodb_1.utils.toDynogelTableDefinition(pubKeysDef) }))
    });
    const friendModel = models['tradle.MyCloudFriend'];
    const friendsDef = definitions.FriendsTable.Properties;
    db.setExclusive({
        model: friendModel,
        table: dynamodb_1.createTable(friendsDef.TableName, Object.assign({}, commonOpts, { exclusive: true, model: friendModel, tableDefinition: dynamodb_1.utils.toDynogelTableDefinition(friendsDef) }))
    });
    return db;
};

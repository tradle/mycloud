...
class Blockchain {
    constructor(tradle) {
        ...
        this.createAdapter = (opts = {}) => {
            ...
            const create = blockchain_adapter_1.default[flavor];
        ...
        this.getWriter = (key) => {
            ...
            if (!this.writers[fingerprint]) {
                ...
                this.writers[fingerprint] = utils_1.promisify(transactor);
            }
        ...
        this.seal = ({ key, link, addresses, counterparty }) => __awaiter(this, void 0, void 0, function* () {
        ...
        this.sealPubKey = (opts) => {
            ...
            link = engine_1.utils.linkToBuf(link);
            basePubKey = engine_1.utils.toECKeyObj(basePubKey);
            return engine_1.protocol.sealPubKey({ link, basePubKey });
        };
        this.sealPrevPubKey = (opts) => {
            let { link, basePubKey } = opts;
            link = engine_1.utils.linkToBuf(link);
            basePubKey = engine_1.utils.toECKeyObj(basePubKey);
            return engine_1.protocol.sealPrevPubKey({ link, basePubKey });
        };
        ...
        if (!blockchain_adapter_1.default[flavor]) {
            throw new Error(`unsupported blockchain type: ${flavor}`);
        }
        this.reader = this.createAdapter();
        this.addressesAPI = utils_1.promisify(this.reader.blockchain.addresses);
        this.getInfo = utils_1.promisify(this.reader.blockchain.info);

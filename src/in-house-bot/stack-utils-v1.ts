...
export default class StackUtils {
  ...
  public static getDeploymentUUIDFromTemplate = (template: CFTemplate) => _.get(template, 'Mappings.deployment.init.deploymentUUID')
  public static getServiceNameFromTemplate = (template: CFTemplate) => _.get(template, 'Mappings.deployment.init.service')
  public static getStageFromTemplate = (template: CFTemplate) => _.get(template, 'Mappings.deployment.init.stage')
  public static getStackNameFromTemplate = (template: CFTemplate) => _.get(template, 'Mappings.deployment.init.stackName')
  public static getServiceNameFromDomain = (domain: string) => domain.replace(/[^a-zA-Z0-9]/g, '-')
  public static getAdminEmailFromTemplate = (template: CFTemplate) => _.get(template, ADMIN_EMAIL_PATH)
  public static getCommitHashFromTemplate = (template: CFTemplate) => _.get(template, 'Resources.Initialize.Properties.commit')
  public static normalizeStackName = (name: string) => /^tdl.*?ltd$/.test(name) ? name : `tdl-${name}-ltd`
  public static setAdminEmail = (template:CFTemplate, email:string) => _.set(template, ADMIN_EMAIL_PATH, email)
  public static setBlockchainNetwork = (template: CFTemplate, value: string) => {
  ...
  public getStackLocation = (versionInfo: VersionInfo) => {
    const { service, stage } = StackUtils.parseStackName(this.thisStackName)
    return StackUtils.getStackLocation({
      service,
      stage,
      versionInfo,

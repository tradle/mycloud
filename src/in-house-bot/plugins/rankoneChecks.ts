...
debugger
    var [photoIdObj, selfieObj] = await Promise.all([this.bot.resolveEmbeds(photoID), this.bot.resolveEmbeds(selfie)])
    // var photoIdObj = await this.bot.resolveEmbeds(photoID)
    // var selfieObj = await this.bot.resolveEmbeds(selfie)
    let photoIdMimeType = this.getContentType(photoID.scan.url)
    let selfieMimeType = this.getContentType(selfie.scan.url)

    const photoIdBuf = DataURI.decode(photoIdObj.scan.url)
    const selfieBuf = DataURI.decode(selfieObj.selfie.url)

    this.appendFileBuf({
      form,
      filename: 'image1',
      content: photoIdBuf,
      contentType: photoIdMimeType
    })
    this.appendFileBuf({
      form,
      filename: 'image2',
    ...
    //   return { status: false, rawData, error }
    // }
    let status = (rawData.similarity > threshold || DEFAULT_THRESHOLD) ? 'pass' : 'fail'

...
export class Applications implements IHasModels {
  ...
  public revokeProductCertificate = async ({ req, certificate }: {
    ...
    if (!properties.revoked) {
      throw new Errors.InvalidInput(`model ${type} has no property "revoke"`)
    }
    ...
    if (certificate[TYPE] === MY_EMPLOYEE_ONBOARDING) {
      ...
      promiseFireEmployee = Promise.resolve()
    }

    await Promise.all([promiseRevoke, promiseFireEmployee])
    return promiseRevoke
  }
...
const removeRole = (user: IUser, role: string) => {
  if (!user.roles) return false

  const { length } = user.roles
  user.roles = user.roles.filter(r => r === role)
  return user.roles.length < length
}

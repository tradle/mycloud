...
import { IPBMiddlewareContext, IPBMiddleware, IDeploymentReportPayload } from '../types'
...
export const createMiddleware = (lambda:Lambda, opts?:any):IPBMiddleware => {
  ...
  const handlePingback = async (ctx:IPBMiddlewareContext, next) => {
    ...
    const report = _.pick(event, REPORT_PROPS) as IDeploymentReportPayload
    const { conf, deployment } = components
    try {
      const success = await deployment.handleDeploymentReport(report)

...
test(
  ...
  loudAsync(async t => {
    ...
    await bot.lambdas.onresourcestream().handler(
      toStreamItems(bot.tables.Bucket0.name, [
        // queueseal
        {
          value: {
...
test(
  ...
  loudAsync(async t => {
    ...
    await bot.lambdas.onresourcestream().handler(
      toStreamItems(bot.tables.Bucket0.name, [{ value: inbound }]),
      // @ts-ignore
      {
        // #6
        getRemainingTimeInMillis: () => 20000
    ...
    await bot.lambdas
      .onresourcestream()
      .handler(toStreamItems(bot.tables.Bucket0.name, [{ value: outbound }]), {
        // #6
      } as ILambdaAWSExecutionContext)

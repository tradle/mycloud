...
test(
  ...
  loudAsync(async t => {
    ...
    t.same(
      LP.getLogEventKey(parsedLogEvent),
      'logs/1970-01-01/00:00/big-mouth-dev-get-index/17d4646a672daea64385cbdc'
    )
    t.equal(LP.parseLogEvent(rawLogEvent).entries.length, 11)
    t.equal(
      LP.getAlertEventKey(parsedAlertEvent),
      'alerts/12345678902/tdl-example-ltd-dev-us-east-1/1970-01-01/00:00/00-0e3c863613'
    )

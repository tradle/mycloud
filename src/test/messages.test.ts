...
import { extractSigPubKey, withLinks } from '../crypto'
...
test(
  ...
  loudAsync(async t => {
    ...
    const stubLastSeqAndLink = sandbox.stub(messages, 'getLastSeqAndLink').callsFake(() =>
      Promise.resolve({
        seq: nextSeq - 1,
        link: prevMsgLink
      })
...
test(
  ...
  loudAsync(async t => {
    ...
    const stubGetIdentity = sandbox
      .stub(identities, 'getPubKeyMapping')
      .callsFake(mocks.getPubKeyMapping)
    const stubPutObject = sandbox.stub(objects, 'put').callsFake(function(object) {
      t.ok(object[SIG])
    ...
    const stubTimestampInc = sandbox
      .stub(messages, 'assertTimestampIncreased')
      .callsFake(promiseNoop)
